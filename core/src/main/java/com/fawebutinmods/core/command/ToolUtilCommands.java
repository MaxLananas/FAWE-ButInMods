...
import com.fawebutinmods.core.tool.ToolTarget;
...
final class ToolUtilCommands {
    ...
    static Transform parseTransform(Ctx ctx) {
        ...
        return switch (action) {
            case "rotate", "rot" -> Transforms.rotate(com.fawebutinmods.core.math.BlockVector3.ZERO,
                    ctx.doubleArg(1, 90));
            case "flip", "mirror" -> Transforms.flip(com.fawebutinmods.core.math.BlockVector3.ZERO,
                    switch (ctx.arg(1, "north").toLowerCase(Locale.ROOT)) {
                        case "x", "east", "west" -> com.fawebutinmods.core.transform.Axis.X;
                        case "y", "up", "down" -> com.fawebutinmods.core.transform.Axis.Y;
                        default -> com.fawebutinmods.core.transform.Axis.Z;
                    });
            case "scale" -> Transforms.scale(com.fawebutinmods.core.math.BlockVector3.ZERO,
    ...
    private void scroll() {
        ...
        entry.handler = ctx -> {
            ...
            com.fawebutinmods.core.tool.Scroll.Action action = com.fawebutinmods.core.tool.Scroll.action(name);
            if (action == null) {
                throw CommandRegistry.error("Scroll action must be one of: "
                        + com.fawebutinmods.core.tool.Scroll.actions());
            }
            ...
            if (action == com.fawebutinmods.core.tool.Scroll.Action.MASK
                    || action == com.fawebutinmods.core.tool.Scroll.Action.PATTERN) {
            ...
            com.fawebutinmods.core.tool.Scroll scroll =
                    com.fawebutinmods.core.tool.Scroll.of(action, brush, ctx.session(), masks, patterns);
            brush.settings().setScrollAction(scroll);
            brush.settings().setScrollActionName((name + " " + String.join(" ", rest)).trim());
            if (action == com.fawebutinmods.core.tool.Scroll.Action.NONE || scroll == null) {
                ctx.actor().message(Msg.success("Scroll action unset"));
            } else {
    ...
    private void featurePlacer() {
        ...
        entry.handler = ctx -> {
            var tool = new com.fawebutinmods.core.tool.Tools.FeaturePlacerTool(ctx.arg(0));
            com.fawebutinmods.core.tool.Tools.bind(ctx.session(), tool, ctx.actor(), null);
    ...
    private void structurePlacer() {
        ...
        entry.handler = ctx -> {
            var tool = new com.fawebutinmods.core.tool.Tools.StructurePlacerTool(ctx.arg(0));
            com.fawebutinmods.core.tool.Tools.bind(ctx.session(), tool, ctx.actor(), null);
