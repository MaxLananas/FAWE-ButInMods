...
final class AnvilCommands {
    ...
    private void copy() {
        ...
        entry.handler = ctx -> {
            ...
            BlockArrayClipboard clipboard = Clipboards.copy(ctx.world(),
                    new com.fawebutinmods.core.region.CuboidRegion(origin, corner), session, false);
    ...
    private void deleteAllOldRegions() {
        ...
        entry.handler = ctx -> {
            long millis = com.fawebutinmods.core.util.Str.parseDuration(ctx.arg(0));
    ...
    private boolean holdsUnknownBlocks(com.fawebutinmods.core.util.NbtCompound chunk) {
        if (chunk == null) {
            return false;
        }
        for (com.fawebutinmods.core.util.NbtCompound section : chunk.getCompoundList("sections")) {
            com.fawebutinmods.core.util.NbtCompound states = section.getCompoundOrNull("block_states");
            if (states == null) {
                continue;
            }
            for (Object entry : states.getList("palette")) {
                if (entry instanceof com.fawebutinmods.core.util.NbtCompound compound) {
